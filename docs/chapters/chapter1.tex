...
W niniejszym rozdziale przedstawiono teoretyczne podstawy zagadnienia, analizę dostępnych na rynku rozwiązań oraz zdefiniowano wymagania stawiane tworzonej aplikacji.
    ...
    \section{Podstawy teoretyczne}
     ...
     Projekt łączy w sobie zagadnienia z dwóch dziedzin: mechaniki nieba oraz grafiki komputerowej. W kontekście astronomicznym, ruch ciał niebieskich w symulacji opiera się na uproszczonym modelu keplerowskim \cite{book_bate_astrodynamics}. Pozycja planety w przestrzeni trójwymiarowej wyznaczana jest na podstawie parametrów orbitalnych, takich jak: promień orbity, okres obiegu, inklinacja (nachylenie orbity względem płaszczyzny odniesienia) oraz długość węzła wstępującego.

     % Rysunek 1.1
    \begin{figure}[htpb]
        \centering
        \includegraphics[width=0.7\textwidth]{images/kepler_elements.png} 
        \caption{Wizualizacja klasycznych elementów orbitalnych Keplera określających pozycję i orientację orbity w przestrzeni 3D}
    ...
    \begin{figure}[htpb]
        \centering
        \includegraphics[width=0.9\textwidth]{images/opengl_pipeline.png}
        \caption{Uproszczony schemat programowalnego potoku graficznego (Programmable Pipeline) w bibliotece OpenGL 4.1}
    ...
    \section{Przegląd istniejących rozwiązań}
     ...
     Na rynku istnieje wiele zaawansowanych symulatorów kosmosu, takich jak \var{SpaceEngine}, \var{Universe Sandbox}, \var{Celestia} czy \var{Kerbal Space Program}. 
     Programy te oferują fotorealistyczną grafikę oraz zaawansowane modele fizyczne, w tym numeryczną symulację grawitacji n-ciał (jak w przypadku \var{Universe Sandbox}).
     Większość współczesnych projektów tego typu opiera się jednak na potężnych, wielofunkcyjnych silnikach komercyjnych (np. Unity czy Unreal Engine), co wiąże się ze znacznym narzutem wydajnościowym i warstwą abstrakcji, która ukrywa przed programistą niskopoziomowe mechanizmy działania potoku graficznego i zarządzania pamięcią operacyjną.

     Niniejszy projekt wyróżnia się na ich tle podejściem ściśle edukacyjnym i inżynierskim. 
     W przeciwieństwie do gotowych rozwiązań, aplikacja „SOLAR” to autorski silnik stworzony całkowicie od podstaw w standardzie języka C, operujący bezpośrednio na API OpenGL 4.1.
     Zamiast obciążającej obliczeniowo symulacji grawitacji n-ciał, zastosowano tu precyzyjny, deterministyczny model oparty na prawach Keplera.
     Takie podejście gwarantuje wysoką wydajność, stabilność wielogodzinnych symulacji oraz umożliwia natychmiastowe przewidywanie pozycji ciał niebieskich w dowolnym momencie czasu (nawet przy ekstremalnym przyspieszeniu symulacji), co jest rozwiązaniem wysoce zoptymalizowanym i w pełni wystarczającym dla celów interaktywnej wizualizacji.
    ...
    \begin{enumerate}
        ...
        \item \textbf{Węzeł wstępujący (Ascending node):} punkt na orbicie, w którym ciało niebieskie przecina płaszczyznę odniesienia, przechodząc z półkuli południowej na północną. Jeden z parametrów pozycjonujących planetę w przestrzeni 3D (pole \var{long_asc_node} w strukturze \var{planet\_t}).
    \end{enumerate}
    ...
    \begin{enumerate}
        ...
        \item \textbf{Manipulacja czasem:} Użytkownik może przyspieszać czas (do skali lat na sekundę) oraz cofać, obserwując zmiany położenia planet.
